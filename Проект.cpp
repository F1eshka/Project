...
void CreatField(int speed, int X, int num_color) //Функция благодоря которой будет отрисовываться игровое поле
{
	...
	SetColor(Black, Color(num_color)); //Смена цвета (буква - чёрный; заливка - жёлтый/синий)
	...
	for (size_t i = 0; i < 12; i++) //выполнеям нижеуказанный код 12 раз так как общее количество строк равно 12
	{
		...
		for (size_t g = 0; g < 23; g++) //выполнеям нижеуказанный код 23 раз так как общее количество столбцов равно 23
		{
			...
			{
				SetColor(Color(num_color), Color(num_color)); //Меняем цвет текста и заливки на жёлтый/синий (границы поля на которых нету цифр ил букв)
...
void Meny(int num)
{
	char variant[11][40] = { "ПИРАТ ДУМАЕТ КУДА УДАРИТЬ...","ПИРАТ ПРОМАЗАЛ!", "ПИРАТ ПОПАЛ!", "ПИРАТ ПОБЕДИЛ. ТОЛЬКО НЕ ПЛАЧЬ :(", "ТВОЙ ХОД." , "ТЫ ПРОМАЗАЛ...", "ТЫ ПОПАЛ!", "ТЫ ПОБЕДИЛ, ПОЗДРАВЛЯЮ!", "+++++++++++++++++++++++++++++++++++++++", "ТЫ ПОПАЛ, ХОДИ СНОВА!", "ЮХУ, ТЫ УНИЧТОЖИЛ ВРАЖЕСКИЙ КОРАБЛИК!" };
...
{
	sf::SoundBuffer BlusterV1Buffer; BlusterV1Buffer.loadFromFile("C:\\Users\\Rog\\Desktop\\ConsoleApplication4\\ProjectMusic\\one.wav");
	sf::Sound BlasterV1(BlusterV1Buffer);
	BlasterV1.setVolume(20);
	BlasterV1.play();

	sf::SoundBuffer BlusterV2Buffer; BlusterV2Buffer.loadFromFile("C:\\Users\\Rog\\Desktop\\ConsoleApplication4\\ProjectMusic\\sea.wav");
	sf::Sound BlasterV2(BlusterV2Buffer);
	BlasterV2.setVolume(25);
	BlasterV2.play();

	sf::SoundBuffer BlusterV3Buffer; BlusterV3Buffer.loadFromFile("C:\\Users\\Rog\\Desktop\\ConsoleApplication4\\ProjectMusic\\Mouse.wav");
	sf::Sound BlasterV3(BlusterV3Buffer);
	BlasterV3.setVolume(30);

	sf::SoundBuffer BlusterV4Buffer; BlusterV4Buffer.loadFromFile("C:\\Users\\Rog\\Desktop\\ConsoleApplication4\\ProjectMusic\\music.wav");
	sf::Sound BlasterV4(BlusterV4Buffer);
	BlasterV4.setVolume(30);

	sf::SoundBuffer BlusterV5Buffer; BlusterV5Buffer.loadFromFile("C:\\Users\\Rog\\Desktop\\ConsoleApplication4\\ProjectMusic\\rick.wav");
	sf::Sound BlasterV5(BlusterV5Buffer);
	BlasterV5.setVolume(30);
	...
	*/
	...
	Sleep(20);
	CreatField(1, 35, Yellow); //создаём игровое поле
	...
	PaintPos(enemy_ship_pos, 37, Yellow); //перерисовываем игровую локации с учетём внесенных изменений (поставили кораблик)
	if (over == 2) //
	{
		Meny(7);
	}
